; ---------------------------------------------------------------------------
; This code is based on the `inflate.asx` by Piotr Fusik. 
;
...
; ---------------------------------------------------------------------------
;
; Ported to ACME assembler by Hugo van Galen on 30th December, 2019.
;
; ---------------------------------------------------------------------------
; (w) Hugo van Galen, 2019
; ---------------------------------------------------------------------------

; NOTE: Zero-page constants moved to `inflate_const.asm`.
...
getBit_return
    ;inc $D800+39; change colour of character here...
	rts
...
!ifdef ZEROPAGE_LOAD_VECTOR {
    jmp (ZEROPAGE_LOAD_VECTOR) ; jmp load_animate
}
...
inflate_end:
    !warn "Total size of code is ", inflate_end-inflate, " bytes";	
...
controlSymbolCodeLength
	!fill CONTROL_SYMBOLS, $00

; Huffman trees
nBitCode_clearFrom
nBitCode_literalCount
    !fill 2*TREE_SIZE, $0  ; org	*+2*TREE_SIZE
...
codeToLiteralSymbol
	!fill 256, $0          ; org	*+256
codeToControlSymbol
	!fill CONTROL_SYMBOLS, $0  ; org	*+CONTROL_SYMBOLS

	
inflate_data_end:
    !warn "Total size of data is ", inflate_data_end-inflate_data, " bytes";	
